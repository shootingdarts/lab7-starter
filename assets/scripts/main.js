...
function getRecipesFromStorage() {
	...
	let recipeArr = JSON.parse(recipes);
	return recipeArr;
}
...
function saveRecipesToStorage(recipes) {
	...
	//            header. It is possible in only a single line, but should
	//            be no more than a few lines.
	let storage = window.localStorage.getItem('recipes');
	let currRecipes = JSON.parse(storage);
	let allRecipes = currRecipes.concat(recipes);
...
function initFormHandler() {
	...
	form.addEventListener('submit', () => {
		...
		saveRecipesToStorage([recipeObject]);
	});
	// B10. TODO - Get a reference to the "Clear Local Storage" button
	let clearStorage = document.querySelector('.danger');
